...
exports.postprocessTransformRawTapeToFinal = postprocessTransformRawTapeToFinal;
...
*/
function postprocessTransformRawTapeToFinal(rawTape, tokenMapping) {
...
function treeNodeFromRawUnsafeInPlace(root, tokenMapping) {
    ...
    switch (root.kind) {
        case parser_types_ts_1.ConstraintTreeNodeKind.OR:
        case parser_types_ts_1.ConstraintTreeNodeKind.AND:
            Object.freeze(root.children);
            break;
        case parser_types_ts_1.ConstraintTreeNodeKind.NOT:
            root.child = root.children[0];
            delete root.children;
            break;
        case parser_types_ts_1.ConstraintTreeNodeKind.ATOM:
            root.tokenIdx = tokenMapping[root.range[0]];
            delete root.children;
            break;
        case parser_types_ts_1.ConstraintTreeNodeKind.IMPLICIT:
            delete root.children;
            break;
        default:
...
function collapseTreeStageOne(root) {
    ...
    for (;;) {
        ...
        const isParenthesis = workingNode.kind == parser_types_ts_1.ConstraintTreeNodeKind.PARENS;
...
function collapseTreeStageTwo(root) {
    ...
    for (;;) {
        ...
        const pairOfNots = (workingNode.kind == parser_types_ts_1.ConstraintTreeNodeKind.NOT
            &&
                workingNode.children[0].kind == parser_types_ts_1.ConstraintTreeNodeKind.NOT);
        if (!pairOfNots) {
            break;
        }
...
function collapseTreeStageThree(root) {
    ...
    if (root.kind != parser_types_ts_1.ConstraintTreeNodeKind.OR
        && root.kind != parser_types_ts_1.ConstraintTreeNodeKind.AND) {
        return;
    }
